...
import { processProposalNotificationQueue } from '@/lib/proposals';
...
async function handleProcess(request: Request) {
    ...
    const limit = await checkRateLimit(`proposal_queue_process_${ip}`, 30, 60_000);
    ...
    if (!expectedSecret) {
        return withNoStore(
            toApiResponse(new ApiError(500, 'SERVICE_MISCONFIGURED', 'Proposal sync secret is not configured.', undefined, false)),
        );
    }

    const bearerToken = getBearerToken(request.headers.get('authorization'));
    const headerToken = (request.headers.get('x-proposal-sync-secret') || '').trim();
    ...
    try {
        ...
        const summary = await processProposalNotificationQueue({
            dryRun,
            limit: limitValue,
        });
        ...
        console.error('[Proposal Queue Process] Failed to process notification queue:', redactErrorForLog(error));
