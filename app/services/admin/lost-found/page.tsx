...
import { ArrowLeft, CheckCircle2, Loader2, PackageSearch, RefreshCw, Save, ShieldCheck, XCircle } from 'lucide-react';
    ...
    return (
        <main className="min-h-screen bg-slate-950 text-slate-100">
            <div className="mx-auto max-w-7xl px-4 pb-20 pt-12 sm:px-6 lg:px-8">
                <Link href="/services/admin" className="inline-flex items-center gap-2 text-sm text-slate-300 hover:text-white"><ArrowLeft size={16} /> Back to Operations Deck</Link>
                <header className="mt-10 flex flex-wrap items-end justify-between gap-5">
                    <div><div className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-amber-200"><ShieldCheck size={15} /> Officer moderation</div><h1 className="mt-4 text-4xl font-semibold text-white">Lost and found queue</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">Publish verified CSO bulletins, review student reports, and retire resolved listings.</p></div>
                    <button type="button" onClick={() => void loadItems()} className="inline-flex items-center gap-2 border border-white/15 px-3 py-2 text-sm text-slate-200 hover:border-amber-200/50"><RefreshCw size={15} /> Refresh</button>
                </header>
                {message ? <p role="status" className="mt-6 border border-amber-200/20 bg-amber-200/10 px-4 py-3 text-sm text-amber-100">{message}</p> : null}

                <div className="mt-8 grid gap-8 lg:grid-cols-[300px_minmax(0,1fr)_360px]">
                    <section className="border border-white/10 bg-white/[0.03] p-4" aria-labelledby="queue-heading">
