...
import { redactErrorForLog, sanitizeText } from '@/lib/security';
...
async function queryHubGuidesFromSheets(): Promise<TransparencyGuide[]> {
const { rows: rawRows } = await loadHubGuideRows();
const diagnostics = {
...
const cachedHubGuidesFromSheets = unstable_cache(queryHubGuidesFromSheets, ['public-hub-guides-sheets'], {
    revalidate: 3600,
    tags: [PUBLIC_CACHE_TAGS.hubGuides],
});

const HUB_GUIDES_READ_TIMEOUT_MS = 3000;

function withTimeout<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new ApiError(504, 'HUB_GUIDES_TIMEOUT', 'Student Hub guides are temporarily unavailable.')), timeoutMs);
        promise.then(
            (value) => {
                clearTimeout(timer);
                resolve(value);
...
async function loadHubGuidesFromSource(source: ReturnType<typeof resolvePublicContentSource>): Promise<TransparencyGuide[]> {
    if (source === 'sheet') return cachedHubGuidesFromSheets();
    try {
        return await loadHubGuidesFromDb();
    } catch (error) {
        if (source === 'db') throw error;
        console.warn('[Hub Guides] Neon source unavailable; falling back to Sheets:', redactErrorForLog(error));
        return cachedHubGuidesFromSheets();
    }
}

export async function loadHubGuides(options: { fallbackOnError?: boolean } = {}): Promise<TransparencyGuide[]> {
    const source = resolvePublicContentSource('HUB_GUIDES_SOURCE');
    try {
        return await withTimeout(loadHubGuidesFromSource(source), HUB_GUIDES_READ_TIMEOUT_MS);
    } catch (error) {
        if (options.fallbackOnError) {
            console.warn('[Hub Guides] Public read failed; returning an empty sanitized state:', redactErrorForLog(error));
            return [];
        }
        throw error;
    }
