...
assert(apiSource.includes('extractDriveResourceKey(parsed.toString())'), 'Hub guides API should forward Drive resource keys when validating links.');
...
const driveSource = read(path.join('lib', 'google-drive.ts'));
assert(driveSource.includes('getDriveFileMetadataById(fileId: string, resourceKey?: string)'), 'Drive metadata helper should accept optional resource keys.');
