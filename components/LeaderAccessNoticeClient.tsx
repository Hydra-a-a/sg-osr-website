...
export default function LeaderAccessNoticeClient({ canSwitchToLeaderMode }: { canSwitchToLeaderMode: boolean }) {
    const [dismissed, setDismissed] = useState(false);

    if (dismissed) return null;

    const clearLeaderAttemptCookie = () => {
            ...
            <div className="rounded-2xl border border-amber-300/35 bg-amber-500/14 p-4 text-amber-50 shadow-[0_16px_44px_rgba(120,53,15,0.22)]">
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex shrink-0 gap-2">
                        {canSwitchToLeaderMode && (
                            <button type="button" onClick={handleSwitch} className="rounded-xl bg-amber-300 px-3 py-2 text-xs font-semibold text-slate-900 transition hover:bg-amber-200">
                                Switch to Leader Mode
                            </button>
                        )}
                        <button
                            type="button"
                            onClick={() => { clearLeaderAttemptCookie(); setDismissed(true); }}
                        ...
                        >
                            {canSwitchToLeaderMode ? 'Dismiss' : 'Continue in Student Mode'}
                        </button>
                    </div>
                </div>
            </div>
